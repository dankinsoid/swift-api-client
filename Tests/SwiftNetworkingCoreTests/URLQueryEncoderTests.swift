...
final class URLEncodedFormQueryEncoderTests: XCTestCase {
    ...
    func testThatQueryIsBodyEncodedAndProperContentTypeIsSetForPOSTRequest() throws {
        // Given
        var client = NetworkClient.test.bodyEncoder(.formURL)
    ...
    func testThatQueryIsBodyEncodedButContentTypeIsNotSetWhenRequestAlreadyHasContentType() throws {
        // Given
        var client = NetworkClient.test.header(.contentType, "type").bodyEncoder(.formURL)
...
//    }
    ...
    func testThatManuallyEncodableStructCanBeEncoded() {
        ...
        print(result?.removingPercentEncoding)
        print(expected.removingPercentEncoding)
        XCTAssertEqual(result, expected)
    }
    ...
    func testThatArrayNestedDictionaryBoolValueCanBeEncodedWithIndexInBrackets() {
        ...
        // Then
        XCTAssertEqual(result, "foo%5B0%5D%5Bbar%5D=1&foo%5B1%5D%5Bqux%5D=0&foo%5B2%5D%5Bquy%5D=1")
    }
    
//    func testThatArraysCanBeEncodedWithoutBrackets() {
//        // Given
//        let encoder = FormURLEncoder(arrayEncodingStrategy: .noBrackets)
...
//    }
    ...
    func testThatNonLatinCharactersArePercentEscaped() {
        ...
        // Then
        let expectedParameterValues: Set<String> = [
            "emoji=%F0%9F%98%83",
            "arabic=%D8%A7%D9%84%D8%B9%D8%B1%D8%A8%D9%8A%D8%A9",
            "japanese=%E6%97%A5%E6%9C%AC%E8%AA%9E",
            "french=fran%C3%A7ais",
        ]
        XCTAssertEqual(Set((result ?? "").components(separatedBy: "&")), expectedParameterValues)
    }
